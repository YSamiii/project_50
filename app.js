const KEY='project-xx-scoring-mobile-v3';
const LEGACY_KEYS=['project-xx-scoring-mobile-v2','project50-scoring-mobile-v1','project-challenge-scoring-mobile-v1'];
...
function defaults(){return {days:50,start:localDate(),manifesto:'坚持完成自己的计划。',sortMode:'custom',rules:{required:[
...
function load(){
  try{
    ...
    const d=defaults();
    return {...d,...x,days:normalizeDays(x.days),sortMode:x.sortMode==='score'?'score':'custom',rules:x.rules||d.rules,entries:x.entries||{}};
...
function orderedRules(kind){const arr=[...state.rules[kind]];return state.sortMode==='score'?arr.sort((a,b)=>(+b.score||0)-(+a.score||0)):arr}
...
function editorCard(r,kind,index,total){const manual=state.sortMode==='custom';return `<div class="editor-card" data-kind="${kind}" data-id="${r.id}"><div class="editor-field icon"><label>图标</label><input class="edit-icon" value="${esc(r.icon)}" maxlength="4"></div><div class="editor-field"><label>规则名称</label><input class="edit-text" value="${esc(r.text)}"></div><div class="editor-field"><label>分数</label><input class="edit-score" type="number" min="0" max="100" step="1" inputmode="numeric" value="${+r.score||0}"></div><div class="order-actions"><button class="secondary move-up" ${!manual||index===0?'disabled':''} aria-label="上移">↑ 上移</button><button class="secondary move-down" ${!manual||index===total-1?'disabled':''} aria-label="下移">↓ 下移</button></div><button class="danger remove">删除此规则</button></div>`}
...
function renderRules(){ $('#sortMode').value=state.sortMode;const req=orderedRules('required'),bonus=orderedRules('bonus');$('#requiredEditor').innerHTML=req.map((r,i)=>editorCard(r,'required',i,req.length)).join('');$('#bonusEditor').innerHTML=bonus.map((r,i)=>editorCard(r,'bonus',i,bonus.length)).join('');refreshScoreCheck();$$('.editor-card').forEach(card=>{const arr=state.rules[card.dataset.kind],r=arr.find(x=>x.id===card.dataset.id);card.querySelector('.edit-icon').oninput=e=>{r.icon=e.target.value;save(false)};card.querySelector('.edit-text').oninput=e=>{r.text=e.target.value;save(false)};card.querySelector('.edit-score').oninput=e=>{r.score=Math.max(0,Math.min(100,+e.target.value||0));save(false);refreshScoreCheck()};card.querySelector('.edit-score').onchange=()=>{if(state.sortMode==='score')renderRules()};card.querySelector('.move-up').onclick=()=>moveRule(card.dataset.kind,r.id,-1);card.querySelector('.move-down').onclick=()=>moveRule(card.dataset.kind,r.id,1);card.querySelector('.remove').onclick=()=>{arr.splice(arr.indexOf(r),1);Object.values(state.entries).forEach(e=>delete e.checks[r.id]);save()}})}
...
$('#sortMode').onchange=e=>{state.sortMode=e.target.value==='score'?'score':'custom';save();toast(state.sortMode==='score'?'已按分数由高到低排序':'已切换为自定义排序')};
...
$('#importData').onclick=()=>$('#importFile').click();$('#importFile').onchange=async e=>{try{const x=JSON.parse(await e.target.files[0].text());const d=defaults();state={...d,...x,days:normalizeDays(x.days),sortMode:x.sortMode==='score'?'score':'custom',rules:x.rules||d.rules,entries:x.entries||{}};save();toast('数据已导入')}catch{alert('JSON 文件格式错误')}};
